    ...
    while n < limit:
        ...
        if source[n:n + 8] == 'function' and source[n + 8] not in IDENTIFIER_PART:
            ...
            if not name or mixed or all_inline:  # its a function expression (can be both named and not named)
                ...
                res += ' ' + iname
                inline['%s@%s' % (iname,
                                  name)] = block, args  # here added real name at the end because it has to be added to the func scope
            last = n
        else:
            n += 1
    res += source[last:]
...
if __name__ == '__main__':
    print
    remove_functions('5+5 function n  (functiona ,functionaj) {dsd  s, dsdd}')
