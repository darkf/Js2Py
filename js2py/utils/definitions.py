...
_DecimalLiteral = (
Combine(DecimalIntegerLiteral('int') + '.' + Optional(DecimalDigits('float')) + Optional(ExponentPart('exp'))) |
...
EscapeSequence = CharacterEscapeSequence | Combine(
    '0' + NotAny(DecimalDigit)) | HexEscapeSequence | UnicodeEscapeSequence
...
StringLiteral = Combine('"' + ZeroOrMore(DoubleStringCharacter) + '"') ^ Combine(
